...
def log_dir(request: pytest.FixtureRequest, test_id: str) -> str:
    ...
    logs = os.path.join(
        os.getcwd(), f"logs/{START_TIME}/{request.function.__name__}/{test_id}"
    )
...
def check_requirements(
    scenario: ScenarioCfg, device: ProbeBuilder, generator: GeneratorBuilder
) -> None:
    ...
    if scenario.requirements.speed is not None:
        if not all(
            ifc.speed >= scenario.requirements.speed
            for ifc in device.get_enabled_interfaces()
        ):
            pytest.skip(
        ...
        if not all(
            ifc.speed >= scenario.requirements.speed
            for ifc in generator.get_enabled_interfaces()
        ):
            pytest.skip(
    ...
    if scenario.requirements.protocols and len(scenario.requirements.protocols) > 0:
        if not set(scenario.requirements.protocols).issubset(
            set(device.get_supported_protocols())
        ):
            pytest.skip(
...
def fixture_xfail_by_probe(
    request: pytest.FixtureRequest, test_id: str, device: ProbeBuilder
):
    ...
    if whitelist:
        for mark in request.node.iter_markers():
            ...
            if whitelist_segment and test_id in whitelist_segment:
                request.applymarker(
                    pytest.mark.xfail(run=True, reason=whitelist_segment[test_id] or "")
                )
...
def pytest_runtest_makereport(item: pytest.Function):
    ...
    if report.when == "call":
        ...
        if (
            archive_data == "failed" and report.outcome == "failed"
        ) or archive_data == "always":
