...
def run_validation(
    ...
    report = ValidationModel(key_fmt, norm_references).validate(
        norm_flows, supported, special
    )
...
def test_basic_ok(setup) -> None:
    ...
    report = run_validation(
        key, flows, references, setup.fields_db, setup.normalizer, setup.supported, {}
    )
...
def test_basic_wrong(setup) -> None:
    ...
    result = report.get_result_by_key_and_field(
        ("192.168.56.101", "192.168.56.102", 45422, 443, 6), "packets", 6
    )
    ...
    result = report.get_result_by_key_and_field(
        ("192.168.56.102", "192.168.56.101", 443, 45422, 6), "packets", 4
    )
    assert ValidationField(name="ttl", expected=64) in result.missing

    assert report.get_fields_summary_stats() == ValidationStats(
        ok=17, error=1, missing=2
    )
...
def test_basic_wrong_unsupported(setup) -> None:
    ...
    supported = [
        elem
        for elem in setup.supported
        if elem not in ["ttl", "tcp_flags", "ip_version", "tcp_syn_size"]
    ]
    report = run_validation(
        key, flows, references, setup.fields_db, setup.normalizer, supported, {}
    )
    assert report.is_passing() is True
...
def test_basic_unexpected(setup) -> None:
    ...
    report = run_validation(
        key, flows, references, setup.fields_db, setup.normalizer, SUPPORTED_FIELDS, {}
    )
    assert report.is_passing() is False
...
def test_any_direction_both(setup) -> None:
    ...
    key, references, flows = read_test_files(
        "any-direction-both.yml", "any-direction.yml"
    )
    report = run_validation(
        key, flows, references, setup.fields_db, setup.normalizer, setup.supported, {}
    )
...
def test_any_direction_single(setup) -> None:
    ...
    key, references, flows = read_test_files(
        "any-direction-single.yml", "any-direction.yml"
    )
    report = run_validation(
        key, flows, references, setup.fields_db, setup.normalizer, setup.supported, {}
    )
...
def test_any_direction_none(setup) -> None:
    ...
    key, references, flows = read_test_files(
        "any-direction-none.yml", "any-direction.yml"
    )
    report = run_validation(
        key, flows, references, setup.fields_db, setup.normalizer, setup.supported, {}
    )
    ...
    result = report.get_result_by_key_and_field(
        ("192.168.1.140", "174.143.213.184", 57678, 80, 6), "bytes", 1234
    )
    assert (
        ValidationField(name="http_host", expected="packetlife.net") in result.missing
    )
    assert (
        ValidationField(name="http_url", expected="/images/layout/logo.png")
        in result.missing
    )
    ...
    assert (
        ValidationField(name="http_agent", expected="Wget/1.12 (linux-gnu)")
        in result.missing
    )
    assert (
        ValidationField(name="http_content_type", expected="image/png")
        in result.missing
    )

    result = report.get_result_by_key_and_field(
        ("174.143.213.184", "192.168.1.140", 80, 57678, 6), "bytes", 23041
    )
    assert (
        ValidationField(name="http_host", expected="packetlife.net") in result.missing
    )
...
def test_fixed_direction_ok(setup) -> None:
    ...
    key, references, flows = read_test_files(
        "fixed-direction-ok.yml", "fixed-direction.yml"
    )
    report = run_validation(
        key, flows, references, setup.fields_db, setup.normalizer, setup.supported, {}
    )
...
def test_fixed_direction_wrong(setup) -> None:
    ...
    key, references, flows = read_test_files(
        "fixed-direction-wrong.yml", "fixed-direction.yml"
    )
    report = run_validation(
        key, flows, references, setup.fields_db, setup.normalizer, setup.supported, {}
    )
    ...
    result = report.get_result_by_key_and_field(
        ("192.168.236.146", "192.168.93.24", 60643, 53, 17), "bytes", 67
    )
    ...
    assert (
        ValidationField(name="dns_req_query_name", expected="ia-dns.net")
        in result.missing
    )
    assert (
        ValidationField(name="dns_flags", expected=256, observed=33154) in result.errors
    )

    result = report.get_result_by_key_and_field(
        ("192.168.93.24", "192.168.236.146", 53, 60643, 17), "bytes", 67
    )
    assert ValidationField(name="dns_resp_rcode", expected=2) in result.missing
    assert (
        ValidationField(name="dns_flags", expected=33154, observed=256) in result.errors
    )

    assert report.get_fields_summary_stats() == ValidationStats(
        ok=8, error=2, missing=4, unchecked=4
    )
    assert report.flows_stats == ValidationStats(error=2)
...
def test_fixed_direction_biflow_ok(setup) -> None:
    ...
    key, references, flows = read_test_files(
        "fixed-direction-biflow-ok.yml", "fixed-direction.yml"
    )
    report = run_validation(
        key, flows, references, setup.fields_db, setup.normalizer, setup.supported, {}
    )
...
def test_subfields_one_in_array_wrong(setup) -> None:
    ...
    result = report.get_result_by_key_and_field(
        ("192.168.197.92", "192.168.21.89", 53, 40980, 17), "bytes", 99
    )
    assert (
        ValidationField(
            name="dns_resp_rr.name",
            expected="newportconceptsla.com",
            observed="unexpected.com",
        )
        in result.errors
    )
    assert (
        ValidationField(name="dns_resp_rr.type", expected=1, observed=5)
        in result.errors
    )
    assert (
        ValidationField(name="dns_resp_rr.class", expected=1, observed=2)
        in result.errors
    )
    assert (
        ValidationField(
            name="dns_resp_rr.data", expected="192.168.73.162", observed="192.168.215.1"
        )
        in result.errors
    )
...
def test_subfields_missing(setup) -> None:
    ...
    assert (
        ValidationField(name="dns_resp_rr.name", expected="newportconceptsla.com")
        in result.missing
    )
    ...
    assert (
        ValidationField(name="dns_resp_rr.data", expected="192.168.73.162")
        in result.missing
    )
...
def test_subfields_unexpected(setup) -> None:
    ...
    key, references, flows = read_test_files(
        "subfields-unexpected.yml", "subfields.yml"
    )
    report = run_validation(
        key,
        flows,
    ...
    assert (
        ValidationField(name="dns_resp_rr", observed=unexpected_dns_rdata)
        in result.unexpected
    )
...
def test_alternative_field_values_wrong(setup) -> None:
    ...
    result = report.get_result_by_key_and_field(
        ("192.168.0.113", "209.31.22.39", 3541, 80, 6), "packets", 6
    )
    assert (
        ValidationField(
            name="http_content_type",
            expected=["text/html", "text/html;charset=UTF-8"],
            observed="image/png",
        )
        in result.errors
    )
...
def test_single_annotation(setup) -> None:
    ...
    key, references, flows = read_test_files(
        "single-annotation.yml", "single-annotation.yml"
    )
    report = run_validation(
        key, flows, references, setup.fields_db, setup.normalizer, setup.supported, {}
    )
...
def test_skipped_fields(setup) -> None:
    ...
    assert setup.normalizer.pop_skipped_fields() == {
        "unknown_field_probe",
