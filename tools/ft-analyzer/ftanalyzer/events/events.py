...
def create_event_queue(flow_df: pd.DataFrame) -> list[Event]:
    ...
    if not one_packet_flows.empty:
        events.extend(
            [
                OnePacketFlow(bytes=row.BYTES, packets=row.PACKETS, time=row.START_TIME)
    ...
    if not multi_packet_flows.empty:
        durations = (
            multi_packet_flows["END_TIME"] - multi_packet_flows["START_TIME"] + 1
        ) / 10**3
        ...
        events.extend(
            [
                ...
                for dr, pr, st in zip(
                    data_rates, packet_rates, multi_packet_flows["START_TIME"]
                )
            ]
        )
        ...
        events.extend(
            [
                ...
                for dr, pr, et in zip(
                    data_rates, packet_rates, multi_packet_flows["END_TIME"]
                )
            ]
        )
